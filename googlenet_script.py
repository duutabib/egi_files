...
from keras.applications import InceptionV3 as googlenet
import h5py as h5py
...
model = googlenet(weights='imagenet', include_top = False)
...
##define act model
outs = [ layer.output for layer in model.layers[:159]]
...
max_auc= np.zeros(len(model.layers[:159]))
for layer_index in range(len(model.layers[:159])):
	...
	plt.ylabel('auRoC')
	plt.grid(True)
	plt.title('GoogLeNet'+str(model.layers[layer_index].name)+': layer '+str(layer_index) )
	plt.savefig('/0/abib/cluster_googlenet_plots/googlenetlayer_'+str(layer_index)+'.png')
...
plt.ylabel('auRoC')
plt.grid(True)
plt.title(' GoogLeNet Layer ' )
plt.savefig('/0/abib/cluster_googlenet_plots/googlenetlayer_maxauc.png')


#write the time taken to file.
t_taken= time.time()- start
fh = open("/0/abib/cluster_googlenet_plots/time_taken.txt","w")
fh.write(t_taken)
fh.close()
