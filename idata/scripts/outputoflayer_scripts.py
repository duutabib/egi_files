...
start = time.time()
block3_conv1_list=[]; block4_conv1_list=[]; block5_conv1_list=[]

for fig_path in tqdm(glob.glob('/0/abib/idata/feature_stimulusdata/Figure*[0-9].png' )): #.need to change this line'85
            ...
            for i in tqdm(range(8)):
                for j in tqdm(range(8)):
                    ...
                    if layer =='block3_conv1':
                        block2_conv1_list.append(pattern)
                    elif layer =='block4_conv1':
                        block2_conv1_list.append(pattern)                            
                    else:
                        block5_conv1_list.append(pattern)

print((time.time() -start)/3600)}

#collecting files
numpy.savetxt('fb3c1_list.txt',block3_conv1_list, fmt='%s', delimiter=',')
numpy.savetxt('fb4c1_list.txt',block4_conv1_list, fmt='%s', delimiter=',')
numpy.savetxt('fb5c1_list.txt',block5_conv1_list, fmt='%s', delimiter=',')
