...
##define act model
outs = [ layer.output for layer in model.layers[:132]]
...
max_auc= np.zeros(len(model.layers[:132]))
for layer_index in range(len(model.layers[:132])):
	...
	plt.ylabel('auRoC')
	plt.grid(True)
	plt.title('Xception'+str(model.layers[layer_index].name)+': layer '+str(layer_index) )
	plt.savefig('/0/abib/cluster_xception_plots/xceptionlayer_'+str(layer_index)+'.png')
...
plt.savefig('/0/abib/cluster_xception_plots/xceptionlayer_maxauc.png')


#write the time taken to file.
t_taken= time.time()- start
fh = open("/0/abib/cluster_xception_plots/time_taken.txt","w")
fh.write(t_taken)
fh.close()
